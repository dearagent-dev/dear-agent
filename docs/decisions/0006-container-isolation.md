...
M6.2 added an OS sandbox: `herald.sandbox.BubblewrapSandbox` wraps the harness argv in
...
and pinning three harness runtimes inside Herald's own image is the thing containers exist to
avoid. And bwrap cannot give a harness a different runtime per task; a container can.

The design goal: Herald decides *how* a harness is jailed, the harness's packaging stays the
harness's business (golden rule 3), and the model/endpoint stays configuration (golden rule
5).

## Decision

**Isolation is pluggable. `HERALD_ISOLATION` selects the mechanism: `bwrap` (default) or
`podman`. Container isolation is expressed as per-harness metadata.**

1. **`HERALD_ISOLATION=bwrap|podman`.** `bwrap` keeps today's behaviour (OpenCode jailed by
   `SandboxPolicy`, egress denied by default). `podman` runs the selected harness in a
   container via `herald.sandbox.ContainerSandbox`.
...
3. **Per-harness metadata on `HarnessInfo`:** `image`, `mounts`, `container_env`. A harness
   ...
   `HERALD_HARNESS_IMAGE` or it fails loudly — never silently unsandboxed.
4. **Credentials are mounts, read-only by default.** The same metadata works for any image
   user: a container path starting with `~` resolves against `HERALD_HARNESS_CONTAINER_HOME`
   (default `/root`). `HERALD_HARNESS_MOUNTS` overrides the mounts wholesale
...
- The image is the harness's own distribution channel; reusing it means Herald never vendors a
...
- **`build_runner` gains a branch on `HERALD_ISOLATION`.** Under bwrap only OpenCode is jailed
...
- Whether to ship a documented default image for a subscription harness (Claude Code) or keep
  requiring `HERALD_HARNESS_IMAGE`.
