...
EVENT_SCHEMA_STATEMENTS: tuple[str, ...] = (
    """
    CREATE TABLE IF NOT EXISTS dear_agent_event (
    ...
    """,
    "CREATE INDEX IF NOT EXISTS dear_agent_event_task_idx ON dear_agent_event (task_id, id)",
    "CREATE INDEX IF NOT EXISTS dear_agent_event_kind_idx ON dear_agent_event (kind, at)",
)
...
class PostgresEventLog:
    ...
    def record(self, task_id: str, kind: str, **data: Any) -> None:
        with self._conn.cursor() as cur:
            cur.execute(
                "INSERT INTO dear_agent_event (at, task_id, kind, data) "
    ...
    def read(self, task_id: str) -> list[TaskEvent]:
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT at, task_id, kind, data FROM dear_agent_event "
    ...
    def recent(self, *, limit: int = 50) -> list[TaskEvent]:
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT at, task_id, kind, data FROM dear_agent_event ORDER BY id DESC LIMIT %s",
                (limit,),
            )
            rows = cur.fetchall()
        self._conn.commit()
    ...
    def count(self, kind: str, *, since: datetime) -> int:
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT count(*) FROM dear_agent_event WHERE kind = %s AND at >= %s", (kind, since)
            )
            row = cur.fetchone()
        self._conn.commit()
...
class ErrorBudget:
    ...
    def from_env(cls, env: dict[str, str]) -> ErrorBudget:
        return cls(
            max_failures=int(env.get("DEAR_AGENT_ERROR_BUDGET_FAILURES", "0") or "0"),
            window_seconds=int(env.get("DEAR_AGENT_ERROR_BUDGET_WINDOW", "3600") or "3600"),
        )
