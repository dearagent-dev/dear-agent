...
from dear_agent.cli.context import CliContext
...
def _add_enqueue(subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument(
        "--verify",
        default=None,
        help="command that must pass before the PR (must be on DEAR_AGENT_VERIFY_ALLOW)",
    )
    parser.add_argument(
        "--depends-on",
        default=None,
...
def _handle_enqueue(args: argparse.Namespace, context: CliContext) -> int:
    ...
    transport_id = args.transport_id or f"<manual-{uuid.uuid4().hex}@dear-agent.local>"
    task = Task(
        id=transport_id,
...
def add_sweep_commands(
    ...
    parser.add_argument(
        "--template", default=os.environ.get("DEAR_AGENT_RUNNER_TEMPLATE_CONFIGMAP")
    )
    parser.add_argument("--limit", type=int, default=20)
...
def _handle_sweep(args: argparse.Namespace, context: CliContext) -> int:
    ...
    from dear_agent.dispatch import TaskDispatcher, kubernetes_launcher
    ...
    if not args.template:
        raise RuntimeError("DEAR_AGENT_RUNNER_TEMPLATE_CONFIGMAP is required to dispatch")
    try:
...
def _handle_listen(args: argparse.Namespace, context: CliContext) -> int:
    ...
    from dear_agent.jmap.eventsource import EventSourceListener
    from dear_agent.jmap.trigger import EventSourceLoop, IngestOnChange
...
def add_poll_commands(
    ...
    parser.add_argument(
        "--recipient", default=None, help="reply recipient (default: DEAR_AGENT_RECIPIENT)"
    )
...
def _handle_poll(args: argparse.Namespace, context: CliContext) -> int:
    ...
    recipient = args.recipient or os.environ.get("DEAR_AGENT_RECIPIENT")
...
def add_idle_commands(
    ...
    parser.add_argument(
        "--recipient",
        default=None,
        help="email to send approval requests to (default: DEAR_AGENT_RECIPIENT)",
    )
...
def _handle_idle(args: argparse.Namespace, context: CliContext) -> int:
    ...
    def submit(proposal) -> str | None:
        ...
        transport_id = f"<idle-{digest}@dear-agent.local>"
        task = Task(
            id=transport_id,
            transport_id=transport_id,
...
def add_policy_commands(
    ...
    sync.add_argument("--dir", default=os.environ.get("DEAR_AGENT_POLICY_DIR", "policy"))
...
def _handle_policy_sync(args: argparse.Namespace, context: CliContext) -> int:
    from dear_agent.policy import build_policy_store, sync_policies
...
def add_health_commands(
    ...
    parser.add_argument(
        "--max-running",
        type=int,
        default=int(os.environ.get("DEAR_AGENT_MAX_RUNNING", "1")),
...
def _handle_parse_reply(args: argparse.Namespace, context: CliContext) -> int:
    from dear_agent.approvals_service import parse_reply
...
def add_run_commands(
    ...
    parser.add_argument(
        ...
        choices=["memory", "jmap"],
        default="memory",
        help="transport backend (default: memory); the queue comes from DEAR_AGENT_QUEUE",
    )
    parser.set_defaults(handler=_handle_run)


def _handle_run(args: argparse.Namespace, context: CliContext) -> int:
    from dear_agent.worker_factory import build_transport, build_worker
...
def _handle_redeem(args: argparse.Namespace, context: CliContext) -> int:
    from dear_agent.approvals import ApprovalError, build_approval_store
    from dear_agent.approvals_service import ApprovalReply, ApprovalService
...
def add_decide_commands(
    ...
    parser.add_argument(
        "--decider",
        default=None,
        help="override DEAR_AGENT_DECIDER (rules|jev|openai-compat|none) for this call",
    )
...
def _handle_decide(args: argparse.Namespace, context: CliContext) -> int:
    ...
    from dear_agent.decision.factory import build_decider
    from dear_agent.decision.port import DecisionError
    from dear_agent.decision.router import HUMAN_QUESTION, ROUTE_QUESTION

    if args.decider:
        os.environ["DEAR_AGENT_DECIDER"] = args.decider
    try:
        decider = build_decider()
...
def add_decision_commands(
    ...
    parser.add_argument(
        "--log",
        default=os.environ.get("DEAR_AGENT_DECIDER_LOG"),
        help="path to the decision log (default: DEAR_AGENT_DECIDER_LOG)",
    )
...
def _log_path(args: argparse.Namespace) -> str:
    if not args.log:
        raise RuntimeError("a decision log is required (--log or DEAR_AGENT_DECIDER_LOG)")
    return args.log
...
def _decision_store(args: argparse.Namespace):
    ...
    if target == "postgres":
        ...
        from dear_agent.decision.log import PostgresDecisionLog

        dsn = os.environ.get("DEAR_AGENT_DATABASE_URL")
        if not dsn:
            raise RuntimeError("DEAR_AGENT_DATABASE_URL is required for --log postgres")
        conn = connect(dsn)
        init_schema(conn)
