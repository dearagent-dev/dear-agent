...
class Notifier:
    ...
    def status(
        ...
        *,
        ...
        subject = f"[dear-agent] {status}: {task.subject or task.id}"
    ...
    def request_approval(
        ...
        *,
        ...
        message = OutboundMessage(
            thread_id=task.thread_id,
            subject=f"[dear-agent] approval needed: {task.subject or task.id}",
            body=body,
            headers=self._headers(task, recipient),
        )
        self._transport.send(message)
    ...
    def _headers(task: Task, recipient: str) -> dict[str, str]:
        headers = {"to": recipient, "X-Dear Agent-Task": task.id}
