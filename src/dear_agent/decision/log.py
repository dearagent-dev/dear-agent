...
from dear_agent.decision.port import Decision, Question

DECISION_SCHEMA_STATEMENTS: tuple[str, ...] = (
    """
    CREATE TABLE IF NOT EXISTS dear_agent_decision (
...
class PostgresDecisionLog:
    ...
    def record(self, record: DecisionRecord) -> None:
        with self._conn.cursor() as cur:
            cur.execute(
                "INSERT INTO dear_agent_decision (at, state, questions, answers, label) "
    ...
    def read(self) -> list[DecisionRecord]:
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT at, state, questions, answers, label FROM dear_agent_decision ORDER BY id"
            )
            rows = cur.fetchall()
        self._conn.commit()
        return [
    ...
    def label(self, index: int, label: str) -> bool:
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT id FROM dear_agent_decision ORDER BY id OFFSET %s LIMIT 1", (index,)
            )
            row = cur.fetchone()
            if row is None:
                self._conn.commit()
                return False
            cur.execute("UPDATE dear_agent_decision SET label = %s WHERE id = %s", (label, row[0]))
        self._conn.commit()
        return True
