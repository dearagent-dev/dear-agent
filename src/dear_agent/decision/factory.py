...
from dear_agent.decision.log import DecisionLog, DecisionRecord, DecisionStore
from dear_agent.decision.policy import DeciderPolicy
from dear_agent.decision.port import (
    Answer,
    Decider,
    DeciderInfo,
    Decision,
...
class EnvDeciderCatalog:
    """The deciders this deployment can run, read from ``DEAR_AGENT_DECIDER_*`` (ADR 0004, M7).

    The environment-backed :class:`~dear_agent.decision.port.DeciderCatalog`: the core asks it
    *what exists* and never learns the answer came from variables. The deterministic ``rules``
    ...
    def _system_one(self) -> DeciderInfo | None:
        if not self.env.get("TYPESAFE_API_KEY"):
            return None
        from dear_agent.decision.jev import DEFAULT_ENDPOINT, DEFAULT_MODEL

        return DeciderInfo(
            id="jev",
            protocol=_SYSTEM_ONE,
            endpoint=self.env.get("DEAR_AGENT_DECIDER_ENDPOINT", DEFAULT_ENDPOINT),
            model=self.env.get("DEAR_AGENT_DECIDER_MODEL", DEFAULT_MODEL),
            native_types=True,
            api_key_env="TYPESAFE_API_KEY",
        )

    def _openai_compat(self) -> DeciderInfo | None:
        endpoint = self.env.get("DEAR_AGENT_DECIDER_BASE_URL")
        ...
        return DeciderInfo(
            ...
            endpoint=endpoint,
            model=self.env.get("DEAR_AGENT_DECIDER_MODEL", ""),
            native_types=False,
...
class EnvDeciderProvider:
    ...
    :class:`~dear_agent.decision.jev.JevDecider`, and ``openai-compat`` is the emulated
    :class:`~dear_agent.decision.openai.OpenAICompatibleDecider`. Credentials are resolved here,
    ...
    def _system_one(self, info: DeciderInfo) -> Decider:
        from dear_agent.decision.jev import DEFAULT_ENDPOINT, DEFAULT_MODEL, JevDecider
    ...
    def _openai_compat(self, info: DeciderInfo) -> Decider:
        ...
        if not info.model:
            raise DecisionError("DEAR_AGENT_DECIDER_MODEL is required for an openai-compat decider")
    ...
    def _referer_headers(self) -> dict[str, str]:
        referer = self.env.get("DEAR_AGENT_DECIDER_REFERER")
        if not referer:
            return {}
        return {"HTTP-Referer": referer, "X-Title": "Dear Agent"}


def build_decider() -> Decider | None:
    """Build the configured decider, or ``None`` when disabled (ADR 0004).

    ``DEAR_AGENT_DECIDER`` names the decider to use (``rules`` is the default): the catalog lists
    ...
    :class:`LoggingDecider` when ``DEAR_AGENT_DECIDER_LOG`` is set.
    """
    requested = _normalize(os.environ.get("DEAR_AGENT_DECIDER", _RULES))
    if requested == "none":
        return None

    threshold = float(os.environ.get("DEAR_AGENT_DECIDER_THRESHOLD", DEFAULT_CONFIDENCE_THRESHOLD))
    require_native = os.environ.get("DEAR_AGENT_DECIDER_REQUIRE_NATIVE", "").lower() == "true"
...
def _maybe_log(decider: Decider) -> Decider:
    target = os.environ.get("DEAR_AGENT_DECIDER_LOG")
    if not target:
        return decider
    if target == "postgres":
        ...
        if not dsn:
            raise DecisionError(
                "DEAR_AGENT_DATABASE_URL is required for DEAR_AGENT_DECIDER_LOG=postgres"
            )
        conn = connect(dsn)
        init_schema(conn)
