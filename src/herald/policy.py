...
POLICY_SCHEMA_STATEMENTS: tuple[str, ...] = (
    """
    CREATE TABLE IF NOT EXISTS herald_policy (
...
class PostgresPolicyStore:
    ...
    def list(self) -> list[ProjectPolicy]:
        with self._conn.cursor() as cur:
            cur.execute(
                ...
                "FROM herald_policy ORDER BY project"
            )
            rows = cur.fetchall()
        self._conn.commit()
    ...
    def get(self, project: str) -> ProjectPolicy | None:
        with self._conn.cursor() as cur:
            cur.execute(
                "SELECT project, enabled, repos, base_branch, harness, model, verify_allow, source "
                "FROM herald_policy WHERE project = %s",
                (project,),
            )
            row = cur.fetchone()
        self._conn.commit()
    ...
    def replace_all(self, policies: list[ProjectPolicy]) -> None:
        with self._conn.cursor() as cur:
            cur.execute("DELETE FROM herald_policy")
            for policy in policies:
                cur.execute(
                    """
                    INSERT INTO herald_policy
